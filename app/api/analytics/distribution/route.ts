...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    const rateLimit = await checkRateLimit(`analytics-distribution:${ip}`, { limit: 120, windowMs: 60_000 })
    if (!rateLimit.allowed) {
      return analyticsErrorResponse(429, 'TOO_MANY_REQUESTS', 'Rate limit exceeded.')
    }

    const url = new URL(request.url)
    const parsed = parseAnalyticsDistributionQuery(url.searchParams)
    if (!parsed.success) {
    ...
    const result = await getAnalyticsDistribution({
      scope: {
        companyId,
