...
type ApiErrorCode = 'INVALID_JSON' | 'VALIDATION_ERROR' | 'NOT_FOUND' | 'FORBIDDEN' | 'INTERNAL_ERROR'
...
async function validateInviteToken(inviteToken: string): Promise<NextResponse> {
  if (!inviteToken) {
...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    const inviteToken = asTrimmed((body as Record<string, unknown>).invite)
    return validateInviteToken(inviteToken)
  } catch (error) {
    logServerError('POST /api/forms/invites/validate failed', error)
